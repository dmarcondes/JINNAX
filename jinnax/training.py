...
def train_pinn(data,width,pde,test_data = None,epochs = 100,activation = jax.nn.tanh,lr = 0.001,b1 = 0.9,b2 = 0.999,eps = 1e-08,eps_root = 0.0,key = 0,epoch_print = 100,plot = False,times = 3,d2 = False,save = False,file_name = 'result_pinn'):
    ...
    test_data : dict
        A dictionay with test data for L2 error calculation. Must have the xt data ('xt') and the solution at each point ('u'). Default None for not calculating L2 error
    epochs : int
    ...
    plot : logical
        Whether to plot the resukt from time to time. Default False
    times : int
...
#Process result
def process_result(test_data,u_trained,train_data,plot = True,times = 4,d2 = True,save = False,file_name = 'result_pinn',print = True):
    #Dimension
