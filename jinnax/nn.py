    ...
    with alive_bar(epochs) as bar:
        ...
        for e in range(epochs):
            ...
            if save:
                ...
                pickle.dump({'params': params,'width': width,'time': time.time() - t0},open(file_name + '_epoch' + str(e).rjust(6, '0') + '.pickle','wb'), protocol = pickle.HIGHEST_PROTOCOL)
            #Update alive_bar
            bar()
...
#Process training
def process_training(test_data,forward,epochs,train_data,file_name,bolstering = True,mc_sample = 10000,plot = True,save = False,file_name_save = 'result_pinn',print_res = True,key = 435,ec = 1e-6,lamb = 1):
    """
    ...
    Returns
    -------
    pandas data frame with training tracing
    """
    #Generate keys
    if bolstering:
    ...
    for e in range(epochs):
        ...
        #Bolstering
        if bolstering:
            k = gk.kernel_estimator(jnp.column_stack((train_data['sensor'],train_data['usensor'])),random.PRNGKey(keys[e]),method = "mpe",lamb = lamb,ec = ec)
