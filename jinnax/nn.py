...
#Process training
def process_training(test_data,forward,epochs,train_data,file_name,at_each = 100,bolstering = True,mc_sample = 10000,save = False,file_name_save = 'result_pinn',print_res = True,key = 435,ec = 1e-6,lamb = 1):
    """
    ...
    forward : function
        ...
        Function with the forward pass of the neural network
...
for e in range(epochs):
    if e % at_each == 0 or e == epochs - 1:
        ...
        #Train MSE and L2
        if train_data['sensor'] is not None:
            train_mse = train_mse + [MSE(psi(train_data['sensor']),train_data['usensor']).tolist()]
            train_L2 = train_L2 + [L2error(psi(train_data['sensor']),train_data['usensor']).tolist()]
        else:
        ...
        #Bolstering
        if bolstering:
            kx = gk.kernel_estimator(train_data['sensor'],random.PRNGKey(keys[e]),method = "mpe",lamb = lamb,ec = ec)
            kx = gk.kernel_estimator(jnp.column_stack((train_data['sensor'],train_data['usensor'])),random.PRNGKey(keys[e]),method = "mpe",lamb = lamb,ec = ec)
            bolst = bolst + [gb.bolstering(psi,train_data['sensor'],train_data['usensor'],k,random.PRNGKey(keys[e]),mc_sample = mc_sample).tolist()]
        else:
            bolst = bolst + [None]
...
df = pd.DataFrame(np.column_stack([ep,time,[sensor_sample] * len(ep),[boundary_sample] * len(ep),[initial_sample] * len(ep),[collocation_sample] * len(ep),
train_mse,test_mse,train_L2,test_L2,bolst]),
    columns=['epoch','training_time','sensor_sample','boundary_sample','initial_sample','collocation_sample','train_mse','test_mse','train_L2','test_L2','bolst'])
if save:
