...
def apply_morph_layer(x,type,params,index_x):
    #Apply each operator
    params = jnp.round(jnp.minimum(jax.nn.relu(params),255))
...
def cmnn(type,width,size,shape_x,key = 0):
    ...
    for i in range(len(width)):
        if type[i] == 'supgen' or type[i] == 'infgen':
            params.append(jnp.zeros((width[i],2,size[i],size[i])) + 128)#initializer(k[i,:],(width[i],2,size[i],size[i]),jnp.float32))
        else:
            params.append(jnp.zeros((width[i],1,size[i],size[i])) + 128)#initializer(k[i,:],(width[i],1,size[i],size[i]),jnp.float32))
